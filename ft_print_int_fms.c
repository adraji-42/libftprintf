...
/*   Updated: 2025/12/13 05:58:57 by adraji           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static  t_lens	ft_calc_int_lens(int n, t_flags f)
{
	...
	len.len_nbr = ft_nbr_len_int(n, &f);

	if (f.dot && f.precision > len.len_nbr)
		len.len_zeros = f.precision - len.len_nbr;

	if (f.dot && len.len_nbr > 0)
		f.zero = 0; 

	len.total_content_len = len.len_nbr + len.len_zeros + sign_len;
...
static int	ft_put_int_prefix_and_padding(int n, t_flags f, t_lens *len, t_vars *var)
{
	char	pad_char;
